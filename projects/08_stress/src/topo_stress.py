...
def airy2load(kx,ky,zobs,H,fk,gk,rlam1,rmu1):
    '''
    Compute the displacements due to the loads
    INPUTS:
    kx - x wavenumber
    ky - y wavenumber
    zobs - depth of calculation
    H - thickness of crust
    fk - surface load
    gk - moho load
    rlam1, rmu1 - Lame parameters
    OUTPUTS:
    cub - x displacement
    cvb - y displacement
    cwb - z displacement
    cdwdz - vertical z displacement gradient
    cdudz - vertical x displacement gradient
    cdvdz - vertical y displacement gradient
    '''
    beta = 2*pi*np.sqrt(kx**2 + ky**2)
...
def disp2stress(kx,ky,cu,cv,cw,cdudz,cdvdz,cdwdz,rlam1,rmu1):
    '''
    Compute stresses from the displacement and displacement gradients
    INPUTS:
    kx, ky - x and y wavenumbers
    cu, cv, cw - x, y, z displacements
    cdudz, cdvdz, cdwdz - vertical displacement gradients
    rlam1, rmu1 - Lame parameters
    OUTPUTS:
    cTxx, cTyy, cTzz - normal stress components
    cTxy, cTxz, cTyz - shear stress components
    '''
...
def topo_stress_grd(infile, zobs, H=7, Te=0, rhoc=2900):
    grv = 9.81
    rhom = 3300.0
    rhow = 1025.0
    young = 7.0e10
    rnu = 0.499 # Poisson's ratio, nu->0.5 minimizes deviatoric stress
