...
@generate_profiles () {
    ...
    gmt grdtrack DZ/${ssp} -G${grid} -C50k/0.5k/2k+v \
        -Sm+sDZ/STACKS/${ssp%.xy}_stack.txt > DZ/STACKS/${ssp%.xy}_profiles.txt
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ## 2.0 EXTRACT INDIVIDUAL PROFILES FROM GRDTRACK OUTPUT
    ssp=${ssp%.xy}
    mkdir DZ/STACKS/${ssp}_profiles
    i=0
    awk -v i=${i} -v ssp=${ssp} '{ if ($1 == ">")
        close ("DZ/STACKS/"ssp"_profiles/"ssp"_profile_"i".txt") i++;
            else
        print $0 > ("DZ/STACKS/"ssp"_profiles/"ssp"_profile_"i".txt")
    }' DZ/STACKS/${ssp%.xy}_profiles.txt
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    ## SET WORKING DIR
    wrk_dir="DZ/STACKS/${ssp}_profiles"
