...
X=0c
Y=21c
gmt grdcut ${bathy_grid} ${R} -GIndian_bathy.nc
@plot_grid Indian_bathy.nc bathy.cpt ${R} ${J} ${X} ${Y} I
...
@auto_pick 052a.xy Indian_bathy.nc
@auto_pick 052b.xy Indian_bathy.nc

## CALCULATE CROSS PROFILES
@generate_profiles 052a.xy Indian_bathy.nc
@generate_profiles 052b.xy Indian_bathy.nc

#@generate_profiles 053a.xy Indian_bathy.nc
#@generate_profiles 053b.xy Indian_bathy.nc

## PLOT EVERY 4th CROSS PROFILE
gmt convert STACKS/052a_profiles.txt -Q0:4:1000 |\
    gmt psxy ${R} ${J} -W0.25p,white ${ko} >> ${pf}

## PLOT CROSS PROFILES
gmt convert STACKS/052b_profiles.txt -Q0:4:1000 |\
    gmt psxy ${R} ${J} -W0.25p,white ${ko} >> ${pf}

## PLOT SSPS
gmt psxy 052a.xy ${R} ${J} -W0.5p,white ${ko} >> ${pf}
gmt psxy 052b.xy ${R} ${J} -W0.5p,white ${ko} >> ${pf}

gmt psxy 053a.xy ${R} ${J} -W0.5p,white ${ko} >> ${pf}
gmt psxy 053b.xy ${R} ${J} -W0.5p,white ${ko} >> ${pf}

gmt psxy AUTO_PICK/052a_profiles/052a_auto_picked.longlat ${R} ${J} -W0.2p,pink ${ko} >> ${pf}
gmt psxy AUTO_PICK/052a_profiles/052a_auto_picked.filtered ${R} ${J} -W0.2p,black ${ko} >> ${pf}
gmt psxy AUTO_PICK/052a_profiles/052a.simple ${R} ${J} -W0.2p,grey ${ko} >> ${pf}

gmt psxy AUTO_PICK/052b_profiles/052b_auto_picked.longlat ${R} ${J} -W0.2p,pink ${ko} >> ${pf}
gmt psxy AUTO_PICK/052b_profiles/052b_auto_picked.filtered ${R} ${J} -W0.2p,black ${ko} >> ${pf}
gmt psxy AUTO_PICK/052b_profiles/052b.simple ${R} ${J} -W0.2p,grey ${ko} >> ${pf}

## PLOT LABELS
font="+f10p,Times-Bold,white"
tail -1 052a.xy | awk '{ print $1, $2, "(c)" }' | gmt pstext ${R} ${J} -F${font} -D-0.25c/0.1c -N ${ko} >> ${pf}
tail -1 052b.xy | awk '{ print $1, $2, "(d)" }' | gmt pstext ${R} ${J} -F${font} -D0.25c/0.1c -N ${ko} >> ${pf}
...
X=10c
Y=8c
gmt grdcut ${VGG_grid} ${R} -GIndian_VGG.nc
@plot_grid Indian_VGG.nc Marine_VGG.cpt ${R} ${J} ${X} ${Y}
...
gmt psscale -Dx0.5c/-1c+w6c/0.2c+jML+h+e -CMarine_VGG.cpt -Bf10a20:"VGG (Eotvos)": ${ko} >> ${pf}
##----------------------------------------------------------------------------------------------------------------------
...
### PLOT RAW PROFILES
profile_dir="STACKS/052a_profiles/"
g=15
...
# PLOT RAW PROFILES
profile_dir2="STACKS/052b_profiles"
g=15
